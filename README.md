...
- **Shared:** `PersonBase`, `CrudRepository<T>`, `BaseRepository<T>` (entity + repositorios genéricos)
- **Validación:** class-validator + class-transformer (ValidationPipe global)
- **Error handling:** AllExceptionsFilter global (cambia errores no controlados a `{ success, message, error }`)
...
```
.github/workflows/
  backend-ci.yml      CI: install → lint → build → test
  frontend-ci.yml     CI: install → lint → build
...
backend/src/
  ...
  health/             health check
  common/filters/     ExceptionFilter global (@Catch())
...
| Pipeline | Pasos |
|----------|-------|
| `backend-ci.yml` | `npm ci` → `lint` → `build` → `test` |
| `frontend-ci.yml` | `npm ci` → `lint` → `build` |

Se activan en push a `main`/`develop` y PR a `main`, solo cuando cambian archivos de su directorio o `shared/`.
